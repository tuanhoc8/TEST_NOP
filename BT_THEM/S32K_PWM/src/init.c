...
void CAN_Init(FlexCAN_Typedef *FlexCAN_x,uint16_t id,PORT_Typedef *PORT_x,uint8_t Tx,uint8_t Rx){
	...
	FlexCAN_x->RAMn[ 1*MSG_BUF_SIZE + 0] = 0x04000000; /* Msg Buf 4, word 0: Enable for reception 	*/
	/* EDL,BRS,ESI=0: CANFD not used 				*/
	/* CODE=4: MB set to RX inactive 				*/
	/* IDE=0: Standard ID 							*/
	/* SRR, RTR, TIME STAMP = 0: not applicable 	*/
//#ifdef NODE_A                                   /* Node A receives msg with std ID 0x511 		*/
//	FlexCAN_x->RAMn[ 4*MSG_BUF_SIZE + 1] = 0x14440000; /* Msg Buf 4, word 1: Standard ID = 0x111 		*/
//#else                                           /* Node B to receive msg with std ID 0x555 	*/
//	FlexCAN_x->RAMn[ 4*MSG_BUF_SIZE + 1] = 0x15540000; /* Msg Buf 4, word 1: Standard ID = 0x555 		*/
//#endif
	FlexCAN_x->RAMn[ 4*MSG_BUF_SIZE + 1]= 0xD0<<18;
	FlexCAN_x->RAMn[ 1*MSG_BUF_SIZE + 1]= 0xC0<<18;
